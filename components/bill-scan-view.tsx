...
export function BillScanView({
  ...
  return (
    <div className="flex flex-col gap-8 max-w-4xl">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-xl border border-black/10 bg-zinc-50/60 px-4 py-3 dark:border-white/10 dark:bg-zinc-900/40">
        <div className="flex items-center gap-3">
          <button
            type="button"
            disabled
            className="flex items-center gap-2 rounded-md border border-zinc-200 bg-white px-3 py-1.5 text-xs font-medium text-zinc-400 cursor-not-allowed dark:border-zinc-800 dark:bg-zinc-900 dark:text-zinc-500"
          >
              ...
              <button
                ...
                className={`flex flex-col text-left p-4 rounded-xl border transition-colors ${
                  isSelected
                    ...
                    : "border-black/10 bg-white hover:bg-zinc-50/80 dark:border-white/10 dark:bg-zinc-950 dark:hover:bg-zinc-900/60"
                }`}
              >
                <span className="text-sm font-semibold text-black dark:text-zinc-50">
                  {scenario.label}
                </span>
                <span className="mt-1 text-xs text-zinc-500 line-clamp-2">
      ...
      {selectedScenario && (
        <div className="flex flex-col gap-6 pt-2">
          ...
          {revealStage >= 1 && (
            <div className="rounded-xl border border-black/10 bg-white p-5 dark:border-white/10 dark:bg-zinc-950 transition-opacity duration-300">
              <div className="flex items-start gap-3">
                <div className="rounded-md bg-zinc-100 p-2 text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300 shrink-0">
          ...
          {revealStage >= 2 && (
            <div className="rounded-xl border border-black/10 bg-white p-5 dark:border-white/10 dark:bg-zinc-950 transition-opacity duration-300">
              ...
              <div className="divide-y divide-black/5 dark:divide-white/5 text-sm">
                ...
                <div className="py-2 flex flex-col sm:flex-row sm:justify-between gap-1">
                  <span className="text-zinc-500">Billed customer</span>
                  <span className="font-medium text-black dark:text-zinc-50">
                    {selectedScenario.invoice.buyerName}
                  </span>
                </div>
                <div className="py-2 flex flex-col sm:flex-row sm:justify-between gap-1">
                  <span className="text-zinc-500">Customer tax identifier</span>
                  <span className="font-mono text-xs font-medium text-black dark:text-zinc-50">
                    {selectedScenario.invoice.buyerTaxId}
                  </span>
                </div>
                <div className="py-2 flex flex-col sm:flex-row sm:justify-between gap-1">
                  <span className="text-zinc-500">Invoice number & date</span>
                ...
                <div className="py-2 flex flex-col sm:flex-row sm:justify-between gap-1">
                  <span className="text-zinc-500">Government reference number</span>
            ...
            <div className="rounded-xl border border-black/10 bg-white p-5 dark:border-white/10 dark:bg-zinc-950 transition-opacity duration-300">
              ...
              <div className="divide-y divide-black/5 dark:divide-white/5">
                {selectedScenario.checks.map((check, index) => (
                  ...
                  >
                    <div className="flex flex-col gap-0.5">
                      <span className="font-medium text-black dark:text-zinc-50">{check.label}</span>
            ...
            >
              <div className="flex flex-col gap-4">
                ...
                {selectedScenario.amountAtRisk > 0 && (
                  <div className="pt-2 border-t border-black/10 dark:border-white/10 flex items-center justify-between text-sm">
                    <span className="text-zinc-600 dark:text-zinc-300">Amount at risk</span>
                    <span className="font-semibold text-black dark:text-white">
                ...
                {(selectedScenario.linkedIssueId || selectedScenario.linkedCommitmentId) && (
                  <div className="pt-3 border-t border-black/10 dark:border-white/10 flex flex-wrap items-center gap-4 text-xs">
                    {selectedScenario.linkedIssueId && (
                      <Link
                        href={`/inspector/project/${projectId}/deviations/${selectedScenario.linkedIssueId}`}
                        className="inline-flex items-center gap-1 font-medium text-black dark:text-white underline hover:opacity-80"
                      >
                    ...
                    {selectedScenario.linkedCommitmentId && (
                      <Link
                        ...
                        className="inline-flex items-center gap-1 text-zinc-600 dark:text-zinc-300 underline hover:opacity-80"
                      >
