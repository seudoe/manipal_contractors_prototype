...
export function TrustNote({
  ...
  return (
    <div className="flex items-start gap-2 rounded-lg border border-black/10 bg-zinc-50 p-3 text-sm dark:border-white/10 dark:bg-zinc-900">
      <Icon size={18} className={`mt-0.5 shrink-0 ${toneFor(trustScore)}`} />
      <div>
        <p className="font-medium text-black dark:text-zinc-50">
          {sourceType.replaceAll("_", " ")}
        </p>
        <p className="text-xs text-zinc-500">{trustLabel}</p>
      </div>
    </div>
  );
}
