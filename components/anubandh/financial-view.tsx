  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="rounded-xl border border-indigo-200 bg-white p-6 dark:border-indigo-800/40 dark:bg-slate-900">
        <p className="text-xs font-semibold uppercase tracking-wide text-slate-400">Claim gap</p>
        <p className="mt-1 text-4xl font-extrabold text-slate-900 dark:text-white">
          {financial.claimGap} points
        </p>
        <p className="mt-2 text-sm text-slate-500">
          Claimed <span className="font-semibold text-slate-900 dark:text-slate-50">{financial.claimedPercent}%</span>{" "}
          vs. observed{" "}
          <span className="font-semibold text-slate-900 dark:text-slate-50">{financial.observedPercent}%</span>
        </p>
      </div>
          ...
          <table className="w-full min-w-[560px] border-collapse text-sm">
            ...
            <tbody>
              {financial.tripleEntry.map((row) => (
                ...
                >
                  <td className="px-4 py-3 font-medium text-slate-900 dark:text-slate-50">{row.period}</td>
                  ...
                  <td className="px-4 py-3 font-medium text-slate-700 dark:text-slate-300">{row.pattern}</td>
                  <td className="px-4 py-3 text-xs text-slate-500">{row.interpretation}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <div className="grid grid-cols-2 gap-4 sm:w-80">
        <div className="rounded-xl border border-indigo-200 bg-white p-4 dark:border-indigo-800/40 dark:bg-slate-900">
          ...
          <p className="mt-0.5 text-lg font-semibold text-slate-900 dark:text-slate-50">{financial.cpi.toFixed(2)}</p>
        </div>
        <div className="rounded-xl border border-indigo-200 bg-white p-4 dark:border-indigo-800/40 dark:bg-slate-900">
          <p className="text-xs font-medium text-slate-500">Cost to complete</p>
