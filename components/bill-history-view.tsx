...
export function BillHistoryView({
  ...
  return (
    <div className="flex flex-col gap-6 max-w-6xl">
      ...
      <div className="flex flex-wrap items-baseline gap-8 sm:gap-12 border-b border-indigo-200 pb-5 dark:border-indigo-800/40">
        <div>
          <span className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-slate-50">
            {totalInvoices}
          </span>
          <span className="ml-2.5 text-xs text-slate-500">total invoices</span>
        </div>
        <div>
          <span className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-slate-50">
            Rs {totalValue.toLocaleString("en-IN")}
          </span>
          <span className="ml-2.5 text-xs text-slate-500">total registered value</span>
        </div>
        <div>
          <span className="text-2xl sm:text-3xl font-semibold text-slate-900 dark:text-slate-50">
            {flaggedCount}
          </span>
          <span className="ml-2.5 text-xs text-slate-500">flagged records</span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-3 rounded-xl border border-indigo-200 bg-white p-3 dark:border-indigo-800/40 dark:bg-slate-900">
        <div className="relative flex-1 min-w-[220px]">
          ...
          <input
            ...
            className="w-full rounded-md border border-indigo-200 bg-transparent pl-9 pr-3 py-1.5 text-sm placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-black dark:border-indigo-800/40 dark:focus:ring-white"
          />
        </div>
        ...
        <select
          ...
          className="rounded-md border border-indigo-200 bg-transparent px-3 py-1.5 text-sm dark:border-indigo-800/40"
        >
      ...
      {filteredBills.length === 0 ? (
        ...
        <div className="overflow-x-auto rounded-xl border border-indigo-200 dark:border-indigo-800/40">
          <table className="w-full min-w-[800px] border-collapse text-sm">
            <thead>
              <tr className="border-b border-indigo-200 bg-slate-50/80 text-left text-xs uppercase tracking-wide text-slate-400 dark:border-indigo-800/40 dark:bg-slate-900">
            ...
            <tbody className="divide-y divide-black/5 dark:divide-white/5">
              {filteredBills.map((bill) => (
                <tr
                  ...
                  className="hover:bg-slate-50/80 cursor-pointer transition-colors dark:hover:bg-slate-900/60"
                >
                  <td className="px-4 py-3.5 text-slate-600 dark:text-slate-400 whitespace-nowrap">
                    {bill.invoiceDate}
                  </td>
                  <td className="px-4 py-3.5 font-mono text-xs font-medium text-slate-900 dark:text-slate-50 whitespace-nowrap">
                    {bill.invoiceNumber}
                  </td>
                  <td className="px-4 py-3.5 font-medium text-slate-900 dark:text-slate-50">
                    {bill.supplierName}
                  </td>
      ...
      {selectedBill && (
        <div
          role="dialog"
          aria-modal="true"
          className="fixed inset-0 z-50 flex items-center justify-center bg-indigo-600/40 p-4 backdrop-blur-xs"
        >
          <div
            className="w-full max-w-xl rounded-2xl border border-indigo-200 bg-white p-6 shadow-xl dark:border-indigo-800/50 dark:bg-slate-950 flex flex-col gap-5 max-h-[90vh] overflow-y-auto"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="flex items-start justify-between gap-4 border-b border-indigo-200 pb-4 dark:border-indigo-800/40">
              <div>
              ...
              <button
                ...
                className="rounded-md p-1.5 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-900"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="flex flex-col divide-y divide-black/5 dark:divide-white/5 text-sm">
              <div className="py-2.5 flex justify-between gap-4">
                <span className="text-slate-500">Supplier organization</span>
              ...
              <div className="py-2.5 flex justify-between gap-4">
                <span className="text-slate-500">Intake logging</span>
            ...
            {selectedBill.linkedIssueId && (
              <div className="pt-3 border-t border-indigo-200 dark:border-indigo-800/40 flex items-center justify-between">
                <span className="text-xs text-slate-500">Linked contract deviation</span>
                <Link
                  href={`/inspector/project/${projectId}/deviations/${selectedBill.linkedIssueId}`}
                  className="inline-flex items-center gap-1.5 text-xs font-medium text-slate-900 dark:text-slate-50 underline hover:opacity-80"
                >
            ...
            <div className="pt-2 flex justify-end">
              <button
                type="button"
                onClick={() => setSelectedBill(null)}
                className="rounded-lg border border-indigo-200 px-4 py-2 text-sm font-medium text-slate-700 hover:bg-slate-50 dark:border-indigo-800/40 dark:text-slate-300 dark:hover:bg-slate-900"
              >
                Close details
              </button>
            </div>
