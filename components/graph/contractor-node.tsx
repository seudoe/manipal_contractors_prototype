...
export function ContractorNode({ data, selected }: NodeProps & { data: ContractorNodeData }) {
  return (
    <div
      className={`flex h-full w-full cursor-grab flex-col justify-center gap-1 rounded-lg border bg-white px-3 py-2 shadow-sm transition-shadow hover:shadow-md dark:bg-zinc-900 ${
        selected
    ...
    >
      ...
      {data.features.length > 0 && (
        <p className="truncate text-xs text-zinc-500">{data.features.join(", ")}</p>
      )}
